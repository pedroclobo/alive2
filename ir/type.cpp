...
expr ByteType::getTypeConstraints() const {
  // since size cannot be unbounded, limit it between 8 and 64 bits if undefined
  auto bw = sizeVar();
  auto r = bw != 0;
  if (!defined) {
    r &= bw.ule(64);
    r &= bw.urem(expr::mkUInt(8, 3)) == 0;
  }
  return r;
}

expr ByteType::sizeVar() const {
  return defined ? expr::mkUInt(bitwidth, var_bw_bits) : Type::sizeVar();
}
...
ByteType::refines(State &src_s, State &tgt_s, const StateValue &src,
                  const StateValue &tgt) const {
  expr refined = true;
  unsigned bytes = bits() / Byte::bitsByte();
  for (unsigned i = 0; i < bytes; ++i) {
    Byte src_byte(src_s.getMemory(),
                          src.value.extract((i + 1) * Byte::bitsByte() - 1,
                                            i * Byte::bitsByte()));
    Byte tgt_byte(tgt_s.getMemory(),
                          tgt.value.extract((i + 1) * Byte::bitsByte() - 1,
                                            i * Byte::bitsByte()));
...
expr ByteType::mkInput(State &s, const char *name,
  ...
  if (does_ptr_mem_access) {
    ...
    ptr_var = s.getMemory().mkInput(ptr_name.c_str(), attrs);
  }
...
SymbolicType::SymbolicType(string &&name)
  : Type(string(name)), i(string(name)), b(string(name)), f(string(name)),
    p(string(name)), a(string(name)), v(string(name)), s(string(name)) {}
