...
static unsigned num_bytes(const Type &ty) {
  unsigned n = 0;
  if (auto bty = ty.getAsByteType())
    n += bty->bw() / 8;
  if (auto aty = ty.getAsAggregateType()) {
    for (unsigned i = 0; i < aty->numElementsConst(); ++i)
      n += num_bytes(aty->getChild(i));
  }
  return n;
}
...
static void calculateAndInitConstants(Transform &t) {
  ...
  num_nonlocals_src = num_globals_src + num_ptrinputs + num_byteinputs +
                      num_nonlocals_inst_src + num_inaccessiblememonly_fns +
                      has_null_block;
